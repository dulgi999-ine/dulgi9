...
function handle(ctx) {
  ...
  if (cmd === "가이드" || cmd.startsWith("가이드 ")) return help.guide(cmd.replace(/^가이드\s*/, ""));
  ...
  if (cmd === "출석") {
    ...
    if (bankBal >= 1000 && d.lastInterest[nick] !== today()) {
      interest = Math.min(Math.floor(bankBal * config.BANK_INTEREST_RATE), config.BANK_INTEREST_CAP);
  ...
  if (cmd === "랭킹") {
    const users = Object.entries(data.load().users).sort((a, b) => b[1] - a[1]).slice(0, 10);
  ...
  if (cmd.startsWith("가르치기 ")) {
    ...
    if (key.length < config.TEACH_MIN_KEY || key.length > config.TEACH_MAX_KEY) return "키 길이 제한";
    ...
    if (Object.keys(d.teach[rid]).length >= config.TEACH_MAX_PER_ROOM) return "이 방 가르치기 개수 상한";
    d.teach[rid][key] = val;
    data.save(d);
    return `등록 \`${key}\` → ${val}`;
  }
  ...
  if (cmd === "슬롯") {
    ...
    if (new Set(r).size === 2) { data.addPoints(nick, 80); return `**슬롯**\n${t}\n+80P`; }
    return `**슬롯**\n${t}\n꽝`;
  }
  ...
  if (cmd.startsWith("룰렛 ")) {
    ...
    if (win) { data.addPoints(nick, win); return `룰렛 ${num} (${color})\n선택 ${choice} → 승리 +${win}P`; }
    return `룰렛 ${num} (${color})\n선택 ${choice} → 패배`;
  }
  ...
  if (cmd.startsWith("하이로우 ")) {
    ...
    const high = card >= 8;
    const ok = (["하이", "high"].includes(choice) && high) || (["로우", "low"].includes(choice) && !high);
    if (ok) { const win = Math.floor(bet * 1.85); data.addPoints(nick, win); return `하이로우 카드 ${card}\n승리 +${win}P`; }
    return `하이로우 카드 ${card}\n패배`;
  }
  ...
  if (cmd.startsWith("홀짝 ")) {
    ...
    if (user === res) { const win = Math.floor(bet * 1.9); data.addPoints(nick, win); return `홀짝 ${num}→${res} 승리 +${win}P`; }
  ...
  if (cmd.startsWith("주사위배팅 ")) {
    ...
    if (dice === guess) { const win = bet * 5; data.addPoints(nick, win); return `주사위 🎲${dice} 맞춤! +${win}P`; }
    return `주사위 🎲${dice} (택 ${guess}) 패배`;
  }
  ...
  if (cmd.startsWith("동전배팅 ")) {
    ...
    if (user === res) { data.addPoints(nick, bet * 2); return `동전 ${res} 승리 +${bet * 2}P`; }
    return `동전 ${res} 패배`;
  }
  ...
  if (cmd === "숫자야구") {
    ...
    let a = "";
    while (a.length < 3) { const i = Math.floor(Math.random() * digits.length); a += digits.splice(i, 1)[0]; }
  ...
  if (cmd.startsWith("가위바위보 ")) {
    ...
    if (!["가위", "바위", "보"].includes(user)) return `사용법: \`${config.PREFIX} 가위바위보 [가위/바위/보]\``;
    const bot = rand(["가위", "바위", "보"]);
    const winMap = { 가위: "보", 바위: "가위", 보: "바위" };
    if (user === bot) return ` 가위바위보 ${user} vs ${bot} → 비김`;
    if (winMap[user] === bot) { data.addPoints(nick, 60); return `✊ 가위바위보 ${user} vs ${bot} → 승리 +60P`; }
  ...
  if (cmd === "퀴즈") {
    const qna = [["대한민국 수도는?", "서울"], ["1+1은?", "2"], ["가장 큰 바다?", "태평양"]];
    const [q, a] = rand(qna);
  ...
  if (cmd === "밸런스") {
    const pairs = [["평생 피자만", "평생 치킨만"], ["시간여행", "순간이동"], ["모든 언어", "모든 악기"]];
    const [a, b] = rand(pairs);
    return `**밸런스**\nA. ${a}\nB. ${b}`;
  }

  if (cmd === "미션") {
    const missions = ["오늘 물 한 잔 마시기", "스트레칭 1분", "감사한 일 하나 떠올리기", "방 정리 5분"];
    return `**오늘의 미션**\n${rand(missions)}`;
  }
  ...
  if (cmd.startsWith("타임캡슐 ")) {
    ...
    const d = data.load();
    d.timecapsules.push({ roomId: String(roomId), nick, message, openAt: Date.now() + days * 86400000 });
    data.save(d);
  ...
  if (cmd === "환율") return "환율 (참고)\nUSD ≈ 1,380원\nJPY ≈ 9.2원\nEUR ≈ 1,500원";
  if (cmd === "시간") return `**시간** ${new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}`;
  if (cmd === "요일") {
  ...
  if (cmd.startsWith("어드민로그인 ")) {
    ...
    const d = data.load();
    if (!d.admins.includes(nick)) { d.admins.push(nick); data.save(d); }
    return `관리자 등록: ${nick}`;
  }
  ...
  if (data.isAdmin(nick)) {
    ...
    if (cmd.startsWith("포인트회수 ")) {
      const parts = cmd.split(/\s+/);
      if (parts.length < 3) return "사용법: 포인트회수  금액";
    ...
    if (cmd === "방추가") {
      const d = data.load();
      const rid = String(roomId);
      if (!d.allowedRooms.map(String).includes(rid)) { d.allowedRooms.push(rid); data.save(d); }
      return `방 추가: ${rid}`;
    }
    if (cmd === "방삭제") {
      const d = data.load();
  ...
  if (taught[cmd]) return safety.sanitizeOut(taught[cmd]);
