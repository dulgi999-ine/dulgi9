...
function defaultData() {
  return {
    users: {}, bank: {}, levels: {}, attendance: {}, lastInterest: {},
    admins: [...config.ADMINS], allowedRooms: [...config.ALLOWED_ROOMS],
    cooldowns: {}, teach: {}, recentSpeakers: {}, votes: {}, novels: {}, timecapsules: [],
  };
}
...
function getPoints(nick) { return load().users[nick] || 0; }
function getBank(nick) { return load().bank[nick] || 0; }
...
function checkCooldown(nick, kind = "default") {
  ...
  if (data.cooldowns[key]) {
    ...
    if (remain > 0) return { blocked: true, msg: `쿨다운 ${remain.toFixed(1)}초` };
  }
...
function getRecent(roomId) { return load().recentSpeakers[String(roomId)] || []; }

module.exports = { load, save, getPoints, getBank, getLevel, addPoints, addBank, isAdmin, isRoomAllowed, checkCooldown, pushRecent, getRecent, defaultData };
